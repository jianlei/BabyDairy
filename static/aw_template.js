var AW_TEMPLATE = {
    ...
    'publishBox' :
    '<div class="modal fade alert-box aw-publish-box" id="photo_modal">'+
    '<div class="modal-dialog" id="photo_dialog">'+
    '<div class="modal-content">'+
    '<div class="modal-header">'+
    '<a type="button" class="close icon icon-delete" data-dismiss="modal" aria-hidden="true"></a>'+
    '<h3 class="modal-title" id="myModalLabel">' + _t('为Baby添加日记吧') + '</h3>'+
    '</div>'+
    '<div class="modal-body">'+
    ...
    '<div>'+
    '<textarea maxlength="50" class="form-control" placeholder="' + _t('日记标题') + ' " rows="1" name="question_content" id="photo_title" onkeydown="if (event.keyCode == 13) { return false; }"></textarea>'+
    '</div>'+
    '<textarea name="question_detail" id="photo_urls" class="form-control" rows="4" placeholder="' + _t('日记内容：')
    + '"></textarea>'+
    '<div>'+
    '<textarea maxlength="50" onkeyup="textarea();" class="form-control" placeholder="' + _t('查看价格（单位：nas）') + '" rows="1" name="question_content" id="photo_price" onkeydown="if (event.keyCode == 13) { return false; }"></textarea>'+
    '</div>'+
    '</form>'+
    '</div>'+
    '<div class="modal-footer">'+
    ...
    '<button class="btn btn-large btn-success" onclick="post();">' + _t('提交') + '</button>'+
    '</span>'+
    '</div>'+
    '</div>'+
    '</div>'+
